...
class CourseRecommendationModel:
    ...
    def __init__(self, data: Dict, embedding_dim: int = 64, num_layers: int = 3,
        ...
        self.graph = self.graph_builder.build_homogeneous_graph() # for LightGCN, GCN
    ...
    def _prepare_training_data(self):
        ...
        if self.using_unenrolled_for_test:
            ...
            # Split into train and valid samples only by valid_size, ignore test_size
            self.train_samples, self.valid_samples = train_test_split(
                positive_samples, test_size=self.valid_split, random_state=36
            )
        else:
            ...
            # Split into train and temp (valid + test) samples
            self.train_samples, temp_samples = train_test_split(
                positive_samples, test_size=(self.test_split + self.valid_split), random_state=36
            )
            
            # Split into valid and test samples
            relative_val_size = self.valid_split / (self.test_split + self.valid_split)
            self.valid_samples, self.test_samples = train_test_split(
                temp_samples, test_size=(1 - relative_val_size), random_state=36
            )
