...
class DataGenerator:
    ...
    def generate_data(
        ...
        if is_save_json:
            ...
            save_generated_dataset(dataset, filepath)
            print(f"Dataset saved to {filepath}")
        return dataset

def save_generated_dataset(data: Dict, filepath: str = './data/generated-dataset.json'):
...
def load_generated_dataset(filepath: str = './data/generated-dataset.json') -> Dict:
    """Load dataset from JSON file (try UTF-8, fall back to cp1252)."""
    try:
